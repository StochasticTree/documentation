...
We simulate from a BCF model using "warm-start" samples fit with the grow-from-root algorithm (@he2023stochastic, @krantsevich2023stochastic). This is the default in `stochtree`.
...
```{r}
mu_hat_test <- predict(bcf_model, X = X_test, Z = Z_test, propensity = pi_test, terms = "prognostic_function")
tau_hat_test <- predict(bcf_model, X = X_test, Z = Z_test, propensity = pi_test, terms = "cate")
plot(
...
sigma2_global_samples = bcf_model.extract_parameter("sigma2_global")
plt.plot(sigma2_global_samples)
plt.axhline(sigma_observed, color="blue", linestyle="dashed", linewidth=2)
plt.xlabel("Sample"); plt.ylabel(r"$\sigma^2$"); plt.title("Global variance parameter")
plt.show()
```
