...
```{r}
generate_data <- function(n, p, snr) {
  if (p < 5) {
  ...
  f_X <- 5 * sin(2 * pi * X[, 1]) + g(X) - 2 * X[, 4] * X[, 5]
  eps <- rnorm(n, mean = 0, sd = sqrt(var(f_X) / snr))
  y <- f_X + eps
  return(list(X = X, f_X = f_X, y = y))
}
```
...
def generate_data(n, p, snr):
    ...
    f_X = 5 * np.sin(2 * np.pi * X[:, 0]) + g(X) - 2 * X[:, 3] * X[:, 4]
...
```{python}
def compute_test_train_indices(
    n: int, test_set_pct: float, random_seed: int = None
) -> tuple:
    train_inds, test_inds = train_test_split(
        np.arange(n), test_size=test_set_pct, random_state=random_seed
    )
  ...
  for (i in 1:mc) {
    ...
    bart_model <- bart(
      ...
      general_params = list(
        ...
        sample_sigma2_global = sample_sigma2,
...
# Run simulation
average_coverage_rates = run_coverage_simulation(n, p, snr, num_gfr, num_burnin, num_mcmc, num_trees, max_depth, mc, interval_levels)
```
...
Clearly, this is quite off. Let's try increasing the number of trees to its default (200) and the max depth to its default (10)
