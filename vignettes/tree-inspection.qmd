...
::: {.callout-note}
## Forest access changes in stochtree 0.5.0

The rest of this page reaches the sampled forests directly, through `bart_model$mean_forests` (R) and `bart_model.forest_container_mean` (Python). Those fields are removed in stochtree 0.5.0 — the forests become part of a single C++-based samples object, and accessing the old names raises an error rather than silently returning something different.

From 0.5.0 onward, extract a standalone copy of the forest instead, and call the same methods
on it:

* R: `extractForest(bart_model, "mean")`
* Python: `bart_model.extract_forest("mean")`
:::
